...
//Pipe line path:  com.bgu.agent.sensors.RotationVectorStatistics


// x = X_SIN_THETA_OVER_2
// y = Y_SIN_THETA_OVER_2
// z = Z_SIN_THETA_OVER_2

@Schedule.DefaultSchedule(interval = 5 , duration = 5 * Constants.SECOND)
@RequiredProbes(AccelerometerSensorProbe.class)
@DisplayName("RotationVectorStatistics")
public class RotationVectorStatistics extends SecureBase implements ContinuousProbe, PassiveProbe
{
    ...
    private class ActivityCounter implements DataListener
    {
        ...
        public void onDataReceived(IJsonObject completeProbeUri, IJsonObject data)
        {
            try
            {
                ...
                {
                    Logger.i(RotationVectorStatistics.class, "Get event while not running");
                    try
                    {
                        getRotationVectorProbe().unregisterListener(this);
                        getRotationVectorProbe().unregisterPassiveListener(this);
                    }
                    catch (Throwable t)
                    {
