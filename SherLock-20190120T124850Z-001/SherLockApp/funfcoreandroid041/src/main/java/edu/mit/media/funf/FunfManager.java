...
//		getApplicationContext().registerReceiver(alarmReciver, new IntentFilter("bgu.simon.FunfAlarmReciver"));
//
//	}
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (action == null || ACTION_KEEP_ALIVE.equals(action)) {
            ...
            Log.i(FunfManager.class.toString(), " action internal");
...
//        Log.i(FunfManager.class.toString(), "getComponenentUri: " + new Uri.Builder()
//                .scheme(FUNF_SCHEME)
//                .path(component) // Automatically prepends slash
//                .fragment(action)
//                .build() + "  component " + component + " action: " + action);
    ...
    public class Scheduler {
        public ArrayList probes = new ArrayList();
        ...
        public void set(String type, Uri componentAndAction, Schedule schedule) {
            ...
            Log.i(FunfManager.class.toString(), "type " + type + " uri " + componentAndAction);
            BigDecimal startTime = schedule.getNextTime(previousTime);
            if (startTime != null) {
                long startTimeMillis = System.currentTimeMillis();
                if (componentAndAction.toString().contains("MoriartyProbe")) startTimeMillis += AlarmManager.INTERVAL_HALF_HOUR;
                if (schedule.getInterval() == null || schedule.getInterval().intValue() == 0) {
                    alarmManager.setExact(AlarmManager.RTC_WAKEUP, startTimeMillis, operation);
                } else {
                    if (probes.isEmpty()) {
                        probes.add(componentAndAction.getPath());
                        k = 0;
                    } else if (probes.contains(componentAndAction.getPath())){
                        k = probes.indexOf( componentAndAction.getPath());
                    }else { probes.add( componentAndAction.getPath());
                        k = probes.indexOf( componentAndAction.getPath());
                    }
                        long intervalMillis = TimeUtil.secondsToMillis(schedule.getInterval());
//                    Log.i(FunfManager.class.getCanonicalName(), " Sending broadcast ");
                    //alarmManager.setExact(AlarmManager.RTC_WAKEUP, startTimeMillis, operation);
                    Intent i = new Intent(getBaseContext(), AlarmReciever.class);
                    i.putExtra("type", type);
                    i.putExtra("uricomp", componentAndAction);
                    i.putExtra("interval", intervalMillis);
                    i.putExtra("pi", k+1);
                    PendingIntent pi = PendingIntent.getBroadcast(getBaseContext(), k+1, i, PendingIntent.FLAG_ONE_SHOT);
                    alarmManager.setExact(AlarmManager.RTC_WAKEUP, startTimeMillis, pi);
