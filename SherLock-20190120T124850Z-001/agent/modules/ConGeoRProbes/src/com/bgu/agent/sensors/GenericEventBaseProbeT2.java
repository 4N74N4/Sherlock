 ...
 */
//Pipe line path:  com.bgu.agent.sensors.GenericEventBaseProbeT2

@Probe.DisplayName("GenericEventBaseProbeT2")
@Schedule.DefaultSchedule(interval = 0, duration = 0, opportunistic = true)
public class GenericEventBaseProbeT2 extends SecureBase implements Probe.ContinuousProbe
{
    ...
    protected void secureOnEnable()
    {
        ...
        lastTime = sp.getLong("T1_interval", 0);
        if (lastTime == null)
            lastTime = 0L;
        long diff = nowTime - lastTime;
        Log.d(TAG, "T1 interval: Now: " + nowTime + " last: " + lastTime + " Diff" + diff);
        if (diff > 120000 && lastTime!=0) {
        ...
        lastTime = nowTime;
        sp.edit().putLong("T2_interval", nowTime).commit();
        ...
        if (sensors != null)
        {
            for (String sensor : sensors)
            {
                ...
                {
                    Logger.e(GenericEventBaseProbeT2.class, "Sensor " + sensor + " in generic event not found", e);
                }
            }
        }
        ...
        genericEventReceiver = new BroadcastReceiver()
        {
            ...
            public void onReceive(Context context, Intent intent)
            {
                synchronized (lock)
                {
                    ...
                    {
                        Logger.i(GenericEventBaseProbeT2.class, "Get new event");
                        ...
                        endOfTimeRunnable = new Runnable()
                        {
                            ...
                            public void run()
                            {
                                try
                                {
                                    synchronized (lock)
                                    {
                                        Logger.i(GenericEventBaseProbeT2.class, "Events waiting time over");
                    ...
                    {
                        Logger.i(GenericEventBaseProbeT2.class, "Get new event BUT in prosses");
    ...
    private void sendSensorsData()
    {
        ...
        if (eventsDic != null)
        {
            ...
            Logger.i(GenericEventBaseProbeT2.class, "Send data");
            try
            {
                if(sensorsData != null)
                {
                ...
                {
                    Logger.e(GeneralSampleProbeT2.class, " Generic T2 - Null Data has been detected");
                }
        ...
        public void onDataReceived(IJsonObject probeConfig, IJsonObject data)
        {
            try
            {
                ...
                if (sensorsDataMapping != null && !sensorsDataMapping.contains(sensorName))
                {
                    ...
                    Logger.i(GenericEventBaseProbeT2.class, "Get new data from : " + sensorName);
