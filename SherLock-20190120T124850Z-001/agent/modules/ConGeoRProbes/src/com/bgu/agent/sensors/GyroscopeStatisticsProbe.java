...
//Pipe line path:  com.bgu.agent.sensors.GyroscopeStatisticsProbe
...
@DisplayName("GyroscopeStatisticsProbe")
public class GyroscopeStatisticsProbe extends SecureBase implements ContinuousProbe, PassiveProbe
{
    ...
    protected void secureOnEnable()
    {
        ...
        getGyroscopeProbe().registerPassiveListener(activityCounter);
    }
    ...
    protected void secureOnStart()
    {
        ...
        if (activityCounter != null)
        {
            getGyroscopeProbe().registerListener(activityCounter);
        }
    }
    ...
    protected void secureOnStop()
    {
        ...
        if (activityCounter != null)
        {
            try
            {
                getGyroscopeProbe().unregisterListener(activityCounter);
            }
            catch (Throwable t)
            {
    ...
    protected void secureOnDisable()
    {
        ...
        if (activityCounter != null)
        {
            try
            {
                getGyroscopeProbe().unregisterPassiveListener(activityCounter);
            }
            catch (Throwable t)
            {
    ...
    private GyroscopeSensorProbe getGyroscopeProbe()
    {
        return getGson().fromJson(DEFAULT_CONFIG, GyroscopeSensorProbe.class);
    }
    ...
    private class ActivityCounter implements DataListener
    {
        ...
        public void onDataReceived(IJsonObject completeProbeUri, IJsonObject data)
        {
            try
            {
                ...
                {
                    Logger.i(GyroscopeStatisticsProbe.class, "Get event while not running");
                    try
                    {
                        getGyroscopeProbe().unregisterListener(this);
                        getGyroscopeProbe().unregisterPassiveListener(this);
                    }
                    catch (Throwable t)
                    {
                ...
                float x = data.get(GyroscopeSensorProbe.X).getAsFloat();
                float y = data.get(GyroscopeSensorProbe.Y).getAsFloat();
                float z = data.get(GyroscopeSensorProbe.Z).getAsFloat();
                update(x, y, z);
            }
            catch (Throwable t)
            {
