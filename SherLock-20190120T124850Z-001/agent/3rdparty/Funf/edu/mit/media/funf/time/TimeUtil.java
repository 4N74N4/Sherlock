...
//	}
    ...
    /**
     * updated function to deal with sensors timestamp error returning future timestamp due to utc value instead of Nano
     * @param nanos
     * @return
     */
    public static BigDecimal uptimeNanosToTimestamp(long nanos) {
        ...
        BigDecimal result = TimeUtil._uptimeNanosToTimestamp(nanos);
        //timestamp in future? assume timestamp is in UTC
        //Log.d(LogUtil.TAG, String.format("calculated timestamp %s: now: %s, compare result: %d",  result.toString(), getTimestamp().toString(), result.compareTo(getTimestamp())));
        if(result.compareTo(getTimestamp()) > 0)
            result = BigDecimal.valueOf(nanos, NANO);
        return result;
    }
